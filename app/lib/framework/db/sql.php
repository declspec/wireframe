...
class SQL {
	...
	**/
	function begin() {
        return $this->depth === -1 && !$this->pdo->beginTransaction()
            ? false
            : ++$this->depth;
	}
	...
	*	@param $options array
	**/
	function __construct($f3,$dsn,$user=NULL,$pw=NULL,array $options=NULL) {
		$this->f3 = $f3;
		$this->uuid=$this->f3->hash($this->dsn=$dsn);
		...
		if (isset($parts[0]) && strstr($parts[0],':',TRUE)=='mysql')
			$options+=array(\PDO::MYSQL_ATTR_INIT_COMMAND=>'SET NAMES '.
				strtolower(str_replace('-','',$this->f3->get('ENCODING'))).';');
