...
//! Base structure
class FatFree implements ArrayAccess {
	//@{ Framework details
	const
	...
	**/
	function exists($key,&$val=NULL) {
		...
		return isset($val)?
			TRUE:
			($this->cache->exists($this->hash($key).'.var',$val)?:FALSE);
	}
	...
	**/
	function devoid($key) {
		$val=$this->ref($key,FALSE);
		return empty($val) &&
			(!$this->cache->exists($this->hash($key).'.var',$val) ||
				!$val);
	}
	...
	**/
	function get($key,$args=NULL) {
		...
		if (is_null($val)) {
			...
			if ($this->cache->exists($this->hash($key).'.var',$data))
				return $data;
		}
		return $val;
	}
	...
	**/
	function clear($key) {
		...
		if (!isset($parts[1]) && array_key_exists($parts[0],$this->init))
			...
			if ($this->cache->exists($hash=$this->hash($key).'.var'))
				// Remove from cache
				$this->cache->clear($hash);
		}
	}
	...
	**/
	function status($code) {
		...
		if (PHP_SAPI!='cli')
			header("{$_SERVER['SERVER_PROTOCOL']} {$code} {$reason}", true, $code);
		return $reason;
	}
			...
			if ($this->hive['VERB']!='OPTIONS' &&
				...
				if (!strlen($body)) {
					...
					if (isset($cache) && !error_get_last())
						...
						$cache->set($hash, array(headers_list(),$body,$result),$ttl);
				}
	...
	**/
    ...
    function joinpath($parts) {
        ...
        $joined = implode(DIRECTORY_SEPARATOR, array_map(array($this,'trimpath'), $parts));
        if ($parts[0][0] === "/" || $parts[0][1] === "\\")
            $joined = DIRECTORY_SEPARATOR . $joined;
        return $joined;
    }
...
class Cache {
	...
	function exists($key,&$val=NULL) {
		...
		switch ($parts[0]) {
			...
			case 'xcache':
				...
				break;
			case 'folder':
				$raw=$this->f3->read($parts[1].$ndx);
				break;
		}
		if (!empty($raw)) {
			list($val,$time,$ttl)=(array)$this->f3->unserialize($raw);
	...
	**/
	function set($key,$val,$ttl=0) {
		...
		$data=$this->f3->serialize(array($val,$time,$ttl));
		$parts=explode('=',$this->dsn,2);
		switch ($parts[0]) {
			case 'apc':
			case 'apcu':
			...
			case 'folder':
				return $this->f3->write($parts[1].$ndx,$data);
		}
		return FALSE;
	}
	...
	function load($dsn) {
		if ($dsn=trim($dsn)) {
			...
			elseif (preg_match('/^memcache=(.+)/',$dsn,$parts) &&
				...
				foreach ($this->f3->split($parts[1]) as $server) {
					$port=11211;
		...
		$this->prefix=$this->f3->hash($_SERVER['SERVER_NAME'].$this->f3->get('BASE'));
		return $this->dsn=$dsn;
	}
	...
	*	Class constructor
	*	@return object
    *   @param $f3 FatFree instance
	*	@param $dsn bool|string
	**/
	function __construct($f3,$dsn=FALSE) {
        $this->f3 = $f3;
		if ($dsn)
			$this->load($dsn);
	}
